...
![代码体积](https://img.shields.io/github/languages/code-size/HuangChenning/captionflow) ![代码行数](https://img.shields.io/endpoint?url=https://raw.githubusercontent.com/HuangChenning/captionflow/badges/loc.json&cacheSeconds=3600)
...
| 字幕窗 | 置顶的悬浮字幕窗，可拖动、可调整大小；字号、颜色和背景不透明度可调，可同时显示最近 1–5 条字幕 |
| 快捷键 | 全局快捷键可自定义；默认 ⌃⌥S 开始/停止，⌃⌥H 显示/隐藏字幕窗，⌃⌥M 字幕窗移到下一块屏幕，⌃⌥A 切换音频源 |
| 语音识别 | WhisperKit `base.en`（默认）或 macOS 26 的苹果系统识别，仅英文，本地运行 |
...
2. **识别**：[WhisperKit](https://github.com/argmaxinc/WhisperKit) 在 Mac 本地把英文语音转成文本。macOS 26 上可以在「设置 → 翻译设置」改用苹果系统识别，识别模型由系统下载和管理。
